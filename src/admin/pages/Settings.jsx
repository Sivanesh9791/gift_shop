...
function SectionCard({ icon: Icon, iconCls = 'text-red-600', title, subtitle, children }) {
 return (
 <div className="bg-white border border-[#E4E4E7] rounded-2xl shadow-sm p-6 mb-6">
 <div className="flex items-center gap-3 mb-1">
 {Icon && <Icon size={20} className={iconCls} />}
 <h2 className="text-base font-bold text-[#111111]">{title}</h2>
 </div>
 {subtitle && <p className="text-sm text-[#6B7280] mb-5">{subtitle}</p>}
 {children}
 </div>
 );
}
...
function Field({ label, helper, children }) {
 ...
 {helper && <p className="text-xs text-[#6B7280] mt-1">{helper}</p>}
 </div>
 );
}

const inputCls = 'w-full px-4 py-2 text-sm bg-[#F4F4F5] text-[#111111] rounded-lg focus:outline-none focus:ring-2 focus:ring-red-400';
...
function ToggleSwitch({ checked, onChange }) {
 ...
 type="button"
 onClick={onChange}
 className={`w-11 h-6 rounded-full transition-colors flex-shrink-0 ${checked ? 'bg-red-600' : 'bg-gray-200 '}`}
 >
...
function SaveButton({ onClick, label = 'Save Changes' }) {
 ...
 <button
 onClick={onClick}
 className="mt-4 px-5 py-2 bg-red-600 hover:bg-red-700 text-white text-sm font-medium rounded-xl transition-colors"
 >
 {label}
 </button>
 );
...
function DangerDialog({ open, title, description, confirmLabel = 'Confirm', onConfirm, onCancel }) {
 ...
 <Dialog.Title className="text-lg font-bold text-[#111111] mb-1">{title}</Dialog.Title>
 <p className="text-sm text-[#6B7280] mb-6">{description}</p>
 <div className="flex gap-3">
 <button onClick={onCancel} className="flex-1 px-4 py-2 border border-gray-200 text-gray-700 text-sm rounded-lg hover:bg-gray-50 font-medium transition-colors">
 Cancel
 </button>
...
function PinInput({ label, value, onChange, show, onToggle, error }) {
 ...
 type="button"
 onClick={onToggle}
 className="absolute right-3 top-1/2 -translate-y-1/2 text-[#A1A1AA] hover:text-[#111111]"
 >
...
export default function Settings() {
 ...
 <div className="mb-4 flex items-center gap-3 bg-[#F4F4F5] rounded-xl px-4 py-3">
 <p className="text-sm text-[#6B7280] font-medium">Current PIN:</p>
 <span className="font-mono text-[#111111] text-base tracking-widest">● ● ● ●</span>
 </div>
 ...
 className="px-5 py-2 bg-red-600 hover:bg-red-700 text-white text-sm font-medium rounded-xl transition-colors"
 >
 Update PIN
 </button>
 </div>
 ...
 <p className="text-sm font-medium text-blue-700 mb-1">💡 Tips for a secure PIN:</p>
 ...
 <div>
 <p className="text-sm font-medium text-[#111111]">Admin Dark Mode</p>
 <p className="text-xs text-[#6B7280]">{darkMode ? 'Dark mode is on' : 'Light mode is active'}</p>
 </div>
 </div>
 ...
 <div className="flex items-center gap-4 p-4 bg-[#F4F4F5] rounded-xl">
 <div className="w-8 h-8 rounded-full bg-red-600 flex-shrink-0 ring-2 ring-red-200 " />
 <div>
 <p className="text-sm font-medium text-[#111111]">IndianBliss Rose</p>
 <p className="text-xs font-mono text-[#6B7280]">#f43f5e</p>
 <p className="text-xs text-[#6B7280] mt-0.5">Contact developer to change brand colors</p>
 </div>
 </div>
 </div>
 </SectionCard>
 ...
 <div className="space-y-4">
 {/* Clear orders */}
 <div className="flex items-center justify-between flex-wrap gap-4 p-4 bg-white border border-[#E4E4E7] rounded-xl border border-red-100 ">
 <div>
 <p className="text-sm font-medium text-[#111111]">Clear All Orders</p>
 <p className="text-xs text-[#6B7280]">Permanently delete all order history</p>
 </div>
 <button
 ...
 <div className="flex items-center justify-between flex-wrap gap-4 p-4 bg-white border border-[#E4E4E7] rounded-xl border border-red-100 ">
 <div>
 <p className="text-sm font-medium text-[#111111]">Reset Products to Default</p>
 <p className="text-xs text-[#6B7280]">Reset product catalog to original data</p>
 </div>
 <button
 ...
 <div className="flex items-center justify-between flex-wrap gap-4 p-4 bg-white border border-[#E4E4E7] rounded-xl border border-red-100 ">
 <div>
 <p className="text-sm font-medium text-[#111111]">Admin Logout</p>
 <p className="text-xs text-[#6B7280]">End your current admin session</p>
 </div>
 <button
