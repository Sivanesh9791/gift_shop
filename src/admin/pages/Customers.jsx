...
function CustomerDetailModal({ customer, onClose }) {
 ...
 {/* Header */}
 <div className="sticky top-0 bg-white border border-[#E4E4E7] border-b border-[#E4E4E7] px-6 py-5 z-10">
 <button onClick={onClose} className="absolute top-4 right-4 text-[#A1A1AA] hover:text-[#111111]">
 <X size={22} />
 </button>
 <div className="flex items-center gap-4">
 <div className="w-16 h-16 rounded-full bg-red-600 text-white flex items-center justify-center text-2xl font-bold flex-shrink-0">
 {initials(customer.name)}
 </div>
 <div>
 <Dialog.Title className="text-xl font-bold text-[#111111] leading-tight">
 {customer.name}
 </Dialog.Title>
 ...
 <div key={label} className="bg-[#F4F4F5] rounded-xl p-4 text-center">
 ...
 <p className="text-xs text-[#6B7280] mb-1">Customer Since</p>
 <p className="text-sm font-medium text-[#111111]">{fmtDate(customer.firstOrderDate)}</p>
 </div>
 <div className="bg-[#F4F4F5] rounded-xl p-4">
 <p className="text-xs text-[#6B7280] mb-1">Last Purchase</p>
 <p className="text-sm font-medium text-[#111111]">{fmtDate(customer.lastOrderDate)}</p>
 <p className="text-xs text-[#6B7280]">{daysAgo(customer.lastOrderDate)}</p>
 </div>
 </div>
 ...
 <div className="text-center py-10 text-[#6B7280]">No orders yet</div>
 ) : (
 ...
 <th key={col} className="px-4 py-2.5 text-left text-xs font-semibold text-gray-500 ">{col}</th>
 ))}
 </tr>
 </thead>
 <tbody>
 {customer.orders.map(order => (
 <tr key={order.id} className="border-t border-[#E4E4E7] hover:bg-gray-50 transition-colors duration-150">
 <td className="px-4 py-3 font-mono text-xs text-red-600 font-medium">{order.id}</td>
 <td className="px-4 py-3 text-[#6B7280]">{(order.items || []).length} items</td>
 <td className="px-4 py-3 font-bold text-[#111111]">₹{(order.total || 0).toFixed(2)}</td>
 <td className="px-4 py-3">
 <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${STATUS_BADGE[order.status] || ''}`}>
 {STATUS_LABEL[order.status] || order.status}
 </span>
 </td>
 <td className="px-4 py-3 text-[#6B7280] text-xs">{fmtDate(order.createdAt)}</td>
 </tr>
 ))}
 </tbody>
 </table>
 ...
 <div>
 <p className="text-sm font-medium text-[#111111]">{customer.address.line1}</p>
 {customer.address.line2 && (
 <p className="text-sm text-[#6B7280]">{customer.address.line2}</p>
 )}
 <p className="text-sm text-[#6B7280]">
 {customer.address.city}, {customer.address.postcode}
 </p>
 <p className="text-sm text-[#6B7280]">{customer.address.country}</p>
 </div>
 </div>
 <p className="text-xs text-[#6B7280] mt-3 pl-7">
 Used in {customer.orderCount} order{customer.orderCount !== 1 ? 's' : ''}
 </p>
 </div>
 ) : (
 <p className="text-sm text-[#6B7280]">No address on file</p>
 )}

 {/* Map placeholder */}
 <div className="bg-[#F4F4F5] rounded-xl h-32 flex flex-col items-center justify-center gap-2">
 <MapPin size={24} className="text-[#6B7280]" />
 <p className="text-sm text-[#6B7280]">Map view coming soon</p>
 </div>
 </div>
 )}
...
export default function Customers() {
 ...
 a.href = url;
 a.download = 'giftiny-customers-' + new Date().toISOString().split('T')[0] + '.csv';
 a.click();
 ...
 <p className="text-sm text-[#6B7280]">{customers.length} total customers</p>
 </div>
 <button
 ...
 {/* Card 1 */}
 <div className="bg-white border border-[#E4E4E7] rounded-2xl p-6 shadow-sm flex items-center gap-4">
 <div className="w-12 h-12 rounded-xl bg-blue-50 flex items-center justify-center flex-shrink-0">
 <Users size={22} className="text-blue-600 " />
 </div>
 <div>
 <p className="text-2xl font-bold text-[#111111]">{customers.length}</p>
 ...
 <div>
 <p className="text-2xl font-bold text-[#111111]">₹{totalRevenue.toFixed(2)}</p>
 ...
 <div>
 <p className="text-2xl font-bold text-[#111111]">{vipCount}</p>
 <p className="text-xs text-[#6B7280]">VIP Customers</p>
 ...
 {/* FILTERS TOOLBAR */}
 <div className="bg-white border border-[#E4E4E7] rounded-2xl p-4 shadow-sm space-y-3">
 ...
 <p className="text-lg font-semibold text-[#6B7280]">No customers found</p>
 <p className="text-sm text-[#6B7280] mb-5">Try adjusting your search</p>
 <button
 onClick={clearSearch}
 className="px-4 py-2 bg-red-600 hover:bg-red-700 text-white text-sm rounded-xl font-medium transition-colors"
 >
 Clear Search
 </button>
 </div>
 ...
 <tr
 key={customer.id}
 className="border-b border-[#E4E4E7] hover:bg-gray-50 transition-colors duration-150"
 >
 {/* Customer */}
 ...
 {/* Total spent */}
 <td className="px-5 py-4">
 <p className="text-sm font-bold text-[#111111]">₹{customer.totalSpent.toFixed(2)}</p>
 </td>

 {/* Avg order */}
 <td className="px-5 py-4">
 <p className="text-sm text-[#6B7280]">₹{customer.avgOrderValue.toFixed(2)}</p>
 </td>
 ...
 {/* Last Order */}
 <td className="px-5 py-4">
 <p className="text-sm text-gray-700 ">{fmtDate(customer.lastOrderDate)}</p>
 <p className="text-xs text-[#6B7280]">{daysAgo(customer.lastOrderDate)}</p>
 </td>
 ...
 <a
 href={`mailto:${customer.email}`}
 className="p-2 text-[#6B7280] hover:bg-gray-100 rounded-lg transition-colors duration-150"
 title="Send email"
 >
 <Mail size={16} />
 </a>
