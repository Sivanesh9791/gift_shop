    ...
    <div className="flex flex-col min-h-screen w-full overflow-x-hidden">
      ...
      <section className="relative w-full bg-gradient-to-r from-rose-100 to-amber-100 py-20 px-4 sm:px-6 md:py-32 flex flex-col items-center justify-center text-center overflow-hidden">
        <div className="absolute top-10 right-10 md:right-20 bg-white/80 backdrop-blur shadow-sm rounded-full px-5 py-2 animate-bounce cursor-default select-none hidden sm:flex items-center gap-2">
          ...
          <span className="font-semibold text-slate-800">10,000+ Corporate Clients</span>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
          {corporateCategories.map((cat) => (
            <Link
              key={cat.label}
              to={cat.link}
              className="flex flex-col items-center p-6 bg-white rounded-2xl shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all border border-slate-100"
            >
              <span className="text-4xl mb-3">{cat.icon}</span>
              <span className="font-semibold text-slate-800 text-center text-sm">{cat.label}</span>
            </Link>
          ))}
        </div>
      </section>
